...
export async function startTrial(formData) {
  ...
  if (!user) redirect(`/login?mode=signup&next=${encodeURIComponent(`/start?plan=${planKey}`)}`);
...
export async function changePlan(formData) {
  ...
  const planKey = formData.get("plan")?.toString();
  const plan = PLANS[planKey];
  if (!plan?.polarProductId || planKey === ctx.org.plan) return;
  ...
  await createPolarClient().subscriptions.update({
  ...
  redirect("/settings?plan_change=requested#billing");
}
...
export async function openBillingPortal() {
  ...
  const session = await createPolarClient().customerSessions.create({
    ...
    returnUrl: `${await origin()}/settings#billing`,
  });
...
export async function deleteAccount(_prev, formData) {
  const ctx = await getAppContext();
  if (ctx.role !== "owner") return { error: "Only the account owner can delete the account." };
  ...
  const { supabase } = ctx;
  const { data: docs } = await supabase.from("cred_documents").select("storage_path");
  const paths = (docs ?? []).map((d) => d.storage_path);
  for (let i = 0; i < paths.length; i += 100) {
    await supabase.storage.from(BUCKET).remove(paths.slice(i, i + 100));
  }

  const { error } = await supabase.rpc("cred_delete_organization");
