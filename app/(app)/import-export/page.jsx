...
import { Card, CardHeader, ICONS, PageHeader, buttonClass } from "@/components/app/ui";
...
export default function ImportExportPage() {
  return (
    <div className="flex flex-col gap-8">
      ...
      <Card>
        <CardHeader icon={ICONS.importExport} title="Export" description="CSV files open in Excel or Google Sheets. Exporting is never limited by your plan." />
          ...
          {EXPORTS.map(([view, title, description]) => (
            <li key={view} className="flex flex-col gap-2 px-5 py-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-xs text-ink-700">{description}</p>
              </div>
