...
export function appUrl(path = "/dashboard") {
  const base = (process.env.NEXT_PUBLIC_SITE_URL || "https://sokndall.com").replace(/\/$/, "");
  return `${base}${path}`;
}
...
<tr><td style="padding:24px 28px 8px;font-size:18px;font-weight:700;color:${BRAND}">Sokndall</td></tr>
...
export function renderAlertEmail(orgName, items) {
...
// The Monday digest.
export function renderDigestEmail(orgName, { queue, stalled, expirations }) {
