...
export async function savePractice(_prev, formData) {
  let { supabase, user, org, client, practice } = await getAppContext();
  ...
  if (!org) {
    // Temporary until the billing phase: the account is created here. From
    // then on only the Polar webhook creates organizations (alcance §10.1).
    const { error } = await supabase.rpc("cred_bootstrap_organization", { p_name: values.legal_name });
    if (error) return { error: `Couldn't create your account: ${error.message}` };
    const { data } = await supabase.from("cred_client_orgs").select("id").limit(1).single();
    client = data;
  }
