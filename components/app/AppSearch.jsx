...
import { getSearchIndex } from "@/lib/search-actions";
import { switchClient } from "@/lib/client-actions";
import { Avatar, ICONS, Icon } from "@/components/app/ui";
...
// The index is shared by every search box on the page (laptop bar, phone
// sheet) and kept per client, so switching client fetches that client's.
const cache = new Map();

export default function AppSearch({ onDone, autoFocus = false, showClients = false, indexKey = "default" }) {
  const router = useRouter();
  const [q, setQ] = useState("");
  const [index, setIndex] = useState(() => cache.get(indexKey) ?? null);
  ...
  // Fetched once, shortly after the page settles (or the moment the box is
  // used), then every keystroke filters it in the browser — no waiting.
  // An index older than a minute is refreshed quietly the next time the box
  // is used, so a provider added a moment ago is findable; the old one keeps
  // answering meanwhile.
  function load() {
    const hit = cache.get(indexKey);
    if (hit) setIndex(hit);
    if (loading || (hit && Date.now() - hit.at < 60000)) return;
    setLoading(true);
    getSearchIndex()
      .then((data) => {
        if (data) {
          const entry = { ...data, at: Date.now() };
          cache.set(indexKey, entry);
          setIndex(entry);
        }
      })
  ...
  useEffect(() => {
    setIndex(cache.get(indexKey) ?? null);
    const id = setTimeout(load, 1200);
    return () => clearTimeout(id);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- reload only when the client changes
  }, [indexKey]);
  ...
  const find = (list) => (words.length && list ? list.filter((x) => words.every((w) => norm(x.terms).includes(w))).slice(0, LIMIT) : []);
  const results = index && words.length ? { providers: find(index.providers), payers: find(index.payers), documents: find(index.documents), clients: find(index.clients) } : null;
  ...
  const busy = (loading && !index && q.trim().length > 0) || pending;
  ...
  return (
    <div ref={box} className="relative w-full">
      ...
      {open && q.trim().length >= 1 && (
        <div className="glass absolute inset-x-0 top-12 z-40 overflow-hidden p-1.5">
          {items.length === 0 ? (
            ...
            <ul className="max-h-[22rem] overflow-y-auto">
              {items.map((item, i) => (
                <li key={item.key}>
                  ...
                  >
                    {item.avatar ? (
                      <Avatar name={item.avatar} photo={item.photo} size="sm" />
                    ) : (
                      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[10px] bg-white/80 text-ink-700" aria-hidden="true">
