...
export const getAppContext = cache(async () => {
  ...
  // for a limited member), each with its practice. Archived clients come back
  // for the owner only, and only to be listed apart (restore, export, delete).
  const { data: rows } = await supabaseAll
    .from("cred_client_orgs")
    .select("id, name, created_at, archived_at, cred_practices(*)")
    .eq("org_id", org.id)
    .order("created_at", { ascending: true });
  const all = (rows ?? []).map(({ cred_practices: p, ...c }) => ({
    ...c,
    practice: Array.isArray(p) ? p[0] ?? null : p ?? null,
  }));
  const clients = all.filter((c) => !c.archived_at);
  const archivedClients = membership.role === "owner" ? all.filter((c) => c.archived_at) : [];

  const chosen = (await cookies()).get(ACTIVE_CLIENT_COOKIE)?.value;
  const client = clients.find((c) => c.id === chosen) ?? clients[0] ?? null;
  // Scoped to the active client whenever there is one (a no-op with a single
  // client). Unscoped reads never include archived clients (RLS).
  const supabase = client ? await createClient({ clientOrgId: client.id }) : supabaseAll;
