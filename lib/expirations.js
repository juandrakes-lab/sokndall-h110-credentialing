...
export async function loadExpirations(supabase, filters = {}) {
  ...
  let credQuery = supabase
    ...
    .select("id, type, custom_name, state, number, issuer, coverage, expiration_date, provider_id, cred_providers!inner(id, first_name, last_name, npi, status, client_org_id)")
