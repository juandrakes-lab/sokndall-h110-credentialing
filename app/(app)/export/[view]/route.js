...
import { CREDENTIAL_TYPES, businessDate, credentialLabel, todayISO } from "@/lib/credentials";
...
const VIEWS = {
  ...
  async credentials({ supabase }) {
    ...
    return [
      ...
      (data ?? []).map((c) => [name(c.cred_providers), c.cred_providers?.npi, credentialLabel(c.type), c.state, c.number, c.issuer,
  ...
  async enrollments({ supabase }) {
    ...
    const who = (id) => (id ? (directory ?? []).find((d) => d.user_id === id)?.email : owner?.email) ?? "";
    return [
  ...
  async communications({ supabase }) {
    ...
    const who = (id) => (directory ?? []).find((d) => d.user_id === id)?.email ?? "";
    return [
