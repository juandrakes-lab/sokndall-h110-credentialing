...
const ENROLLMENT_SELECT = `id, provider_id, payer_id, status, next_follow_up_date, submitted_date,
  status_changed_at, assigned_user_id, pending_request, pending_request_at,
  cred_providers!inner(id, first_name, last_name, status, client_org_id),
...
export async function loadFollowUps(supabase, { provider, payer, status } = {}) {
  ...
  const [{ data: queueRows }, { data: stalledRows }, { data: directory }, { data: requestRows }] = await Promise.all([
    narrow(
      supabase
    ...
    // Applications waiting on us for something the payer asked for.
    narrow(supabase.from("cred_enrollments").select(ENROLLMENT_SELECT).eq("status", "info_requested")).order("pending_request_at", {
      ascending: true,
      nullsFirst: true,
    }),
  ]);
  ...
  const requests = (requestRows ?? []).map(shape);

  // Last logged contact per enrollment, for "where did we leave it".
  const ids = [...new Set([...queue, ...stalled, ...requests].map((e) => e.id))];
  ...
  return { queue, stalled, requests, lastContact, directory: directory ?? [] };
}
