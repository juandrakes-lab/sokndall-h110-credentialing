...
const planOf = (sub) => planForProduct(sub.productId) ?? sub.product?.metadata?.plan ?? sub.metadata?.plan;
...
export async function applySubscription(admin, sub, eventType = "subscription.updated") {
  ...
  const { data, error } = await admin.rpc("cred_sync_subscription", {
    ...
    // Billing Co's users on the plan (10 included, $39 each beyond).
    p_seats: sub.seats ?? null,
  });
...
export async function syncFromPolar(admin, userId) {
  ...
  const sub = live
    .filter((s) => ["solo", "practice", "billing_co"].includes(planOf(s)))
