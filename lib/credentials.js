// The credential types of alcance §3.3 (rev. 2026-09-18: + state CDS and
// "Other"). The table has generic columns (custom_name, state, number, issuer,
// coverage, issue_date, expiration_date); this is the one place that says
// which columns a type uses and what they're called.
export const CREDENTIAL_TYPES = {
  ...
  // Controlled substance registration a state issues on top of the DEA.
  cds: {
    label: "State CDS registration",
    fields: ["state", "number", "issue_date", "expiration_date"],
    labels: { state: "State", number: "CDS registration number", issue_date: "Issued", expiration_date: "Expires" },
    required: ["state", "number", "expiration_date"],
  },
  // Anything else with an expiration date (BLS/ACLS, a hospital badge…).
  other: {
    label: "Other",
    fields: ["custom_name", "number", "issue_date", "expiration_date"],
    labels: { custom_name: "What it is", number: "Number", issue_date: "Issued", expiration_date: "Expires" },
    placeholders: { custom_name: "e.g. BLS certification" },
    required: ["custom_name", "expiration_date"],
  },
};
...
export const CREDENTIAL_FIELDS = ["custom_name", "state", "number", "issuer", "coverage", "issue_date", "expiration_date"];

// An "Other" credential is called by the name it was given.
export function credentialLabel(type, credential) {
  if (type === "other") return credential?.custom_name || "Other credential";
